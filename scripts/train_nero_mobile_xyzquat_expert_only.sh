...
LOG_DIR="${LOG_DIR:-/data0/xieweize/checkpoints/lingbotvla/logs}"
RUN_ID="${RUN_ID:-$(date '+%Y%m%d_%H%M%S')}"
TRAIN_LOG="${TRAIN_LOG:-$LOG_DIR/nero_mobile_xyzquat_expert_only_$RUN_ID.log}"
mkdir -p "$(dirname "$TRAIN_LOG")"

echo "Training log: $TRAIN_LOG"
echo "TensorBoard logs: /data0/xieweize/checkpoints/lingbotvla/runs"
...
python -m torch.distributed.run \
  ...
  configs/vla/real_robot/nero_mobile_xyzquat_expert_only.yaml \
  "$@" 2>&1 | tee "$TRAIN_LOG"
